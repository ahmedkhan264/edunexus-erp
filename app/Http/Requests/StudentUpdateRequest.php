...
class StudentUpdateRequest extends FormRequest
{
    ...
    {
        // Only allow admin and principal roles to update students
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'status' => ['required', 'string', 'in:enrolled,graduated,suspended,withdrawn'],
            'is_active' => ['required', 'boolean'],
            'graduation_date' => ['nullable', 'date', 'after:admission_date', 'before_or_equal:today'],
            ...
            'father_cnic' => ['nullable', 'string', 'max:15', 'regex:/^[0-9]{5}-[0-9]{7}-[0-9]{1}$/', 'unique:parent_profiles,father_cnic,' . optional($student->parentProfile)->id],
            'father_phone' => ['nullable', 'string', 'max:20'],
            'father_occupation' => ['nullable', 'string', 'max:100'],
            'father_email' => ['nullable', 'email', 'max:100'],
            
            'mother_name' => ['nullable', 'string', 'max:100'],
            'mother_cnic' => ['nullable', 'string', 'max:15', 'regex:/^[0-9]{5}-[0-9]{7}-[0-9]{1}$/', 'unique:parent_profiles,mother_cnic,' . optional($student->parentProfile)->id],
            'mother_phone' => ['nullable', 'string', 'max:20'],
            'mother_occupation' => ['nullable', 'string', 'max:100'],
            'mother_email' => ['nullable', 'email', 'max:100'],
            
            'guardian_name' => ['nullable', 'string', 'max:100'],
            'guardian_cnic' => ['nullable', 'string', 'max:15', 'regex:/^[0-9]{5}-[0-9]{7}-[0-9]{1}$/', 'unique:parent_profiles,guardian_cnic,' . optional($student->parentProfile)->id],
    ...
    public function messages(): array
    {
        return [
            ...
            'graduation_date.after' => 'Graduation date must be after admission date.',
            'graduation_date.before_or_equal' => 'Graduation date must be today or in the past.',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Cannot deactivate student with unpaid fees (placeholder logic)
            if (!$this->input('is_active') && $student->is_active) {
                // This would check for unpaid fees in a real implementation
                // For now, we'll allow deactivation
            }
            
            // If status is graduated, graduation date is required
            if ($this->input('status') === 'graduated' && !$this->input('graduation_date')) {
                $validator->errors()->add('graduation_date', 'Graduation date is required when status is graduated.');
            }
            
            // If status is graduated, student must be active
            if ($this->input('status') === 'graduated' && !$this->input('is_active')) {
                $validator->errors()->add('is_active', 'Student must be active to mark as graduated.');
            }
        });
    }
}
